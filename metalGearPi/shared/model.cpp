...
Model::Model(std::string const& p_path)
{
    ...
    std::string filename = (lastSlash == std::string::npos) ? p_path : p_path.substr(lastSlash + 1);

    if (!LoadModel(p_path))
    {
        printf("error: model '%s' was unable to be loaded \n", filename.c_str());
    }
    else
    {
...
Model::~Model()
{
    // OpenGL texture cleanup (if Model owns the textures)
    for (const Texture& texture : m_textures_loaded)
    {
        glDeleteTextures(1, &texture.id);
    }

    // The vector `m_meshes` will automatically clean up `Mesh` objects if they manage their resources properly.
    m_meshes.clear(); // Clear the vector to free any additional memory
}



void Model::Draw(BaseShader* p_shader)
{
    #if defined(WINDOWS_BUILD) || defined(LINUX_BUILD)
...
unsigned int Model::TextureFromFile(const char* p_path, const std::string& p_directory, bool)
{
